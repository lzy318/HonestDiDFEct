...
# DELTA^{RM}(Mbar) Identified Set Helper Functions --------------------
.compute_IDset_DeltaRM_fixedS <- function(s, Mbar, max_positive,
  ...
  dir_RM = base::c(dir_RM, base::rep("==", base::NROW(prePeriodEqualityMat)))
...
.compute_IDset_DeltaRM <- function(Mbar, trueBeta, l_vec, numPrePeriods, numPostPeriods) {
...
# Delta^{RM}(Mbar) Inference Helper Functions -------------------------
.computeConditionalCS_DeltaRM_fixedS <- function(s, max_positive, Mbar,
  ...
  if (numPostPeriods == 1) {
    ...
    CI <- .APR_computeCI_NoNuis(betahat = betahat, sigma = sigma,
                                A = A_RM_s, d = d_RM, numPrePeriods = numPrePeriods, numPostPeriods = numPostPeriods,
    ...
    CI = .ARP_computeCI(betahat = betahat, sigma = sigma, numPrePeriods = numPrePeriods,
                        numPostPeriods = numPostPeriods, A = A_RM_s, d = d_RM,
...
computeConditionalCS_DeltaRM <- function(betahat, sigma, numPrePeriods, numPostPeriods,
  ...
  CIs_RM_plus_allS = base::matrix(0, nrow = gridPoints, ncol = base::length(s_indices))
  ...
  for (s_i in 1:base::length(s_indices)) {
    ...
    # Compute CI for s, (-) and bind it to all CI's for (-)
    CI_s_minus = .computeConditionalCS_DeltaRM_fixedS(s = s_indices[s_i], max_positive = FALSE, Mbar = Mbar,
                                                      betahat = betahat, sigma = sigma, numPrePeriods = numPrePeriods,
                                                      numPostPeriods = numPostPeriods, l_vec = l_vec,
                                                      alpha = alpha, hybrid_flag = hybrid_flag, hybrid_kappa = hybrid_kappa,
                                                      postPeriodMomentsOnly = postPeriodMomentsOnly,
  ...
  CIs_RM_plus_maxS = base::apply(CIs_RM_plus_allS, MARGIN = 1, FUN = base::max)
  CIs_RM_minus_maxS = base::apply(CIs_RM_minus_allS, MARGIN = 1, FUN = base::max)
